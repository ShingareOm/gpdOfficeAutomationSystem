...
Class Action {
	...
	function login(){
		extract($_POST);
			$qry = $this->db->query("SELECT *, concat(user_name,' ',user_surname) as name FROM gpd_users where user_email = '".$email."' and user_password = '".md5($password)."'  ");
		if($qry->num_rows > 0){
	...
	function save_user(){
		...
		$check = $this->db->query("SELECT * FROM gpd_users where user_email ='$user_email' ".(!empty($id) ? " and user_id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
